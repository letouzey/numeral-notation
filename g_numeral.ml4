...
let rec constr_of_glob = function
  | {CAst.v = Glob_term.GRef (ConstructRef c, _)} -> mkConstruct c
  | {CAst.v = Glob_term.GApp (gc, gcl)} ->
...
let rec glob_of_constr ?loc c = match Constr.kind c with
  | App (c, ca) ->
      ...
      CAst.make ?loc (Glob_term.GApp (c, cel))
  | Construct (c, _) -> CAst.make ?loc (Glob_term.GRef (ConstructRef c, None))
  | Const (c, _) -> CAst.make ?loc (Glob_term.GRef (ConstRef c, None))
  | Ind (ind, _) -> CAst.make ?loc (Glob_term.GRef (IndRef ind, None))
  | Var id -> CAst.make ?loc (Glob_term.GRef (VarRef id, None))
...
let q_int = qualid_of_string "Decimal.int"
let q_uint = qualid_of_string "Decimal.uint"
...
let has_type ?loc f ty =
  let c =
    CAst.make ?loc
      (CCast (CAst.make ?loc (CRef (f, None)), CastConv ty)) in
  let (sigma, env) = Lemmas.get_current_context () in
  try
...
let vernac_numeral_notation ty f g scope opts =
  ...
  let cty = CAst.make (CRef (ty, None)) in
  let app x y = CAst.make (CApp ((None, x), [(y, None)])) in
  let cref q = CAst.make (CRef (Qualid (None,q), None)) in
  let arrow x y =
    CAst.make
      (CProdN ([([(None, Anonymous)], Default Decl_kinds.Explicit, x)], y))
  in
  let cZ = cref q_z in
